...
### 3.1 `control-case.f` — `CASE` / `OF` / `ENDOF` / `ENDCASE`
...
### 3.5 `defer.f` — `DEFER` / `IS` / `ACTION-OF` / `DEFER@` / `DEFER!`
...
### 3.7 `wordlist-tools.f` — `NAME>INTERPRET` / `NAME>COMPILE` / `TRAVERSE-WORDLIST`
...
### 3.9 `double.f` — `2CONSTANT` / `2VARIABLE` / `2VALUE` / `D.R`
...
### 3.11 `ansi.f` 補齊的 convenience — `INCLUDE` / `BIN` / `FILE-STATUS`
...
### 5.6 `patch.f` — `REPLACE-WORD` 直接改 hot path
...
### 5.7 `onoff.f` — `ON` / `OFF` flag helper
...
### 5.8 `rnd.f` — `RANDOM` / `CHOOSE` / `RANDOMIZE`
...
### 5.9 `uppercase.f` — `UPPERCASE` / `CEQUAL-U` / `CHAR-UPPERCASE`
...
### 5.11 `util.f` — `TryOpenFile` 模組 / library 路徑搜尋
...
### 5.12 `const.f` — 動態常數 vocabulary 機制
...
### 6.1 自動載入：`RENAME-FILE` 走 POSIX 路徑
...
### 7.2 `lib/win/mutex.f` — 跨 process 互斥
...
### 7.4 `lib/win/winerr.f` — `DECODE-ERROR` 與 `WTHROW`
...
### 7.5 `lib/win/const.f` — Windows 常數表
...
### 7.6 `lib/win/api-call/` — 替代 API 呼叫模型
...
### 7.7 `lib/win/spfgui/` — SP-Forth GUI 支援

`spfgui` 是 SP-Forth 的傳統 GUI 工具箱雛形，包含 button、edit、list 等控制項包裝。現代 GUI 開發建議參考 `ac-lib3/win/window/`（[16-ac-lib3.md](16-ac-lib3.md)）。
...
- 大型應用 / prototype → 看 [16-ac-lib3.md](16-ac-lib3.md) 找對應功能。
