...
`spf4e` 使用 `SAVE-WITH-RESERVE` 儲存映像，並額外預留 10 MiB 可用字典空間（`10 1024 * 1024 *`）。這裡用 `S" $@"` 很重要：Linux 目標會展開成 `../spf4e`，Windows/Cygwin 目標則會展開成 `../spf4e.exe`，因此檔名、路徑與副檔名都由 Make 的 target 名稱決定。它會先載入 `lib/ext/spf4e.f`，而該檔再納入「所有可用的標準字（不含 BLOCK word set）」與部分實用延伸庫；因此較精確的理解是 extended executable，而不是單純把整個 `lib/` 目錄無差別全包進去。
...
├── 環境/外部函式：
│   ├── POSIX: src/posix/envir.f, src/posix/defwords.f, src/posix/mtask.f
│   └── Windows: 對應 win/*.f
├── 系統初始化：src/spf_init.f
├── 映像儲存：
│   ├── POSIX: src/posix/save.f → src/xsave.f
│   └── Windows: src/win/spf_pe_save.f → src/tsave.f
├── 向量修復和最終化
└── XSAVE（POSIX）或 DONE（Windows）
```
...
結果格式如 `"25.Apr.2026"`。建構系統使用此字詞產生 `BUILD-DATE` 常數。
...
: StartColonHelp ( flag.is-primitive -- )
  ...
  GET-CURRENT ... S" vocabulary" ATTRIBUTE-OUT  \ 所屬詞彙表
...
0 ,     \ 旗標
' elf-header-size EXECUTE W,   \ ELF 標頭大小（0x34）
' section-size   W,            \ 段標頭大小（0x28 在 xsave.f 中為 0x20）
sections#       W,             \ 段數量
1                W,             \ 段名稱字串表段索引
```
...
: (SAVE-WITH-RESOURCES) ( u.offset.section-resources sd.filename-exe -- )
  ...
  \ 修改 PE 標頭中的欄位
  ?Res IF 3 ELSE 2 THEN HERE START-PE-HEADER 0x06 + + W!  \ NumberOfSections
...
POSIX 版的 `done.f` 不需要特別處理，因為映像儲存由 `XSAVE` 完成。但 `spf.f` 中的最終化步驟：
