...
> 本章目標：看懂 C-CALL 如何從 Forth 堆疊推入 x86 堆疊、(( / )) 語法怎麼呼叫外部函數、信號如何轉成 THROW。
> 
...
> **數值僅供參考，非 SP-Forth 寫死的常數。** 以下 §1.3–§1.5 列出的具體數值，是 `posix/config.c` 透過 `DEFINE(...)`（直接取系統標頭的 `#define` 值，如 `SA_RESTART`、`SIGSEGV`、`S_IFREG`）或 `CONST(..., offsetof/sizeof ...)`（如 `STAT_ST_MODE`、`SIZEOF_SIGSET`）在**目標平台**編譯期計算出來的。表中數值對應典型的 32 位元 glibc Linux，會隨 libc / 核心 / 架構不同而改變。若要確認當前系統的實際值，請查產生出來的 `posix/config.auto.f`，不要把這些數字當成跨平台保證。

| 常數 | 值（典型 32-bit glibc Linux） | 說明 |
|------|-----|------|
| `SIGILL` | 0x4 | 不合法指令 |
| `SIGSEGV` | 0xB | 記憶體存取違規 |
| `SIGBUS` | 0x7 | 匯流排錯誤 |
...
呼叫前堆疊（TOS-in-EAX 模型）：
  ...
  [EBP]   = n（引數數量）
  [EBP+4] = xn（最靠近 TOS 的引數）
  [EBP+8] = x(n-1)
  ...           （Forth 堆疊效果為 ( x1 ... xn n adr -- res )）

1. 由 xn 往 x1 依序 PUSH 到 x86 堆疊
   因 x86 stack 向下成長，C 函式最終看到的參數順序仍是 x1, x2, ..., xn（符合 cdecl）

2. CALL EAX（呼叫 C 函數）
   回傳值在 EAX 中

3. 清理 x86 堆疊（cdecl 由呼叫者清理）
   ADD ESP, n*4

4. 調整 Forth 資料堆疊
   ADD EBP, (n+1)*4
   （移除 n 與 n 個引數；原本在 EAX 的函式位址已被 C 回傳值取代，成為新的 TOS）
```
...
`<(` 是 `((` 的替代語法，用於明確指定引數數量（字名是 `<(`，見 `posix/dl.f:209`）。例如 PROCESSPROC: 使用 `2 <(` 來指定 2 個引數。
...
: ALLOCATE-THREAD-MEMORY ( -- )
  ...
  IF
     ...
     R> R@ TlsIndex@ CELL- ! >R \ 在 TLS 區塊前寫入回返位址（?）
  ELSE
     -300 THROW                  \ 記憶體分配失敗
  THEN
;
...
3. 呼叫 `exit(ERRNUM)`，以傳入的錯誤碼終止程序

`1 <( )) exit` 是 C 函數呼叫語法：`1` 代表傳遞一個引數給 C 的 `exit` 函數；實際引數是 `HALT` 進入時資料堆疊上的 `ERRNUM`，不是常數 1。
...
`CREATE-FILE` 呼叫 `open64(path, fam | O_CREAT | O_TRUNC, 0644)`，支援大檔案（>2 GiB）。注意 `0644`（0x1A4）是新檔案的**權限模式**（第三個參數），不是 open flags；flags 是 `fam | O_CREAT | O_TRUNC`。
`OPEN-FILE` 直接使用 `open64`，參數為檔案名稱和 FAM。
...
`FILE-EXIST` 使用 `__xstat`（或 `stat`）系統呼叫檢查檔案是否存在。`FILE-EXISTS` 進一步**排除目錄**（檢查 `st_mode & S_IFDIR` 為 0）；注意它只排除目錄，並不嚴格保證是 regular file——裝置檔、FIFO 等也可能通過。
...
`compile-call`（第 126~135 行）編譯外部函數呼叫：
...
`PAUSE` 的實作將毫秒轉換為 `timespec` 結構（秒 + 奈秒）：`1000 UM/MOD` 取得秒與剩餘毫秒，剩餘毫秒再乘以 `1000000` 轉成 `tv_nsec`，然後呼叫 `nanosleep`。
...
處理器會**嘗試**把 SIGSEGV/SIGILL/SIGBUS/SIGFPE 轉成 Forth `THROW`；但能否真的恢復，取決於外層是否有有效的 `CATCH`、USER/TLS 狀態是否仍可用，以及信號是否發生在可安全恢復的位置。若有有效 `CATCH` 才可能回到 Forth 例外流程，否則仍會走致命錯誤處理。因此不應把它理解成「永遠不會崩潰、必定可恢復」。
...
`SA_SIGINFO` 使得信號處理器接收三個引數（含 siginfo_t），而不僅僅是信號碼。`SA_NODEFER` 的語意是「處理該信號時不自動遮蔽同一信號」，因此它**允許**同一信號重入；它不是用來「防止遞迴死鎖」，反而需要搭配 `IN-EXCEPTION` 等保護來避免重入造成的傾印遞迴。
...
5. **POOL-INIT**：設定資料/回返堆疊基準（`S0`/`R0`）與執行緒局部執行狀態（`BASE`、`TIB`、`SOURCE-ID`、搜尋順序、`HANDLER` 等），**不**負責分配堆疊記憶體
...
> 本章聚焦 SAVE 在 POSIX 平台層的實作細節（ELF 節區標頭表佈局、重定位、gcc 連結）；關於 SAVE 在建構流程與自舉脈絡中的角色，另見 [06-build-save.md §7](06-build-save.md#7-posix-映像儲存posixsavef--elff)。
...
: SAVE ( c-addr u -- )
  ...
  HERE FORTH-START - DUP            \ 計算 .forth 節區大小
  sections 5 elf-section-size * + 5 CELLS + !       \ 更新節區標頭表
  
  ... 更新 .space, .dltable, .dlstrings 節區 ...
  
  sections 9 elf-section-size * R@ WRITE-FILE THROW \ 寫出節區標頭表
...
### 14.2 ELF 節區標頭表（Section Header Table）結構

> **術語說明**：ELF 使用 **Section（節區）** 作為連結檢視的基本單位，透過「節區標頭表」描述。這與執行檢視的 **Segment（段）** 不同——後者由「程式標頭表」描述，用於作業系統載入。SP-Forth 產生的 `.o` 檔案主要包含節區資訊，最終可執行檔的段資訊由 `gcc` 連結器決定。
...
\ 節區 7：.dltable（動態連結表節區）
45 , 1 , 0x3 , 0 , 0 , 0 , 0 , 4 , 0

\ 節區 8：.dlstrings（動態連結字串節區）
54 , 3 , 0x2 , 0 , 0 , 0 , 0 , 4 , 0
```

每個節區標頭項目包含 10 個 32 位元欄位（40 bytes），符合 ELF32 節區標頭格式：

| 偏移 | 欄位 | 說明 |
|------|------|------|
| +0 | sh_name | 名稱偏移（.shstrtab 中） |
| +4 | sh_type | 節區類型（SHT_PROGBITS 等） |
| +8 | sh_flags | 節區旗標 |
| +12 | sh_addr | 虛擬位址 |
| +16 | sh_offset | 檔案偏移 |
| +20 | sh_size | 節區大小 |
...
其中：
  S = 符號值（Symbol Value）
      來自符號表（.symtab）中對應符號的 st_value 欄位
      例如：dlopen 符號的值 = 0（外部未定義符號，由動態連結器解析）

  A = 加數（Addend）
      重定位位置在重定位前的原始值
      對於 REL 類型（非 RELA），加數隱含為重定位位置的記憶體內容
```

**SP-Forth 重定位範例**（`dlopen` 函數指標）：

```forth
\ 重定位項目定義（save.f:166-198）
' dlopen-adr >BODY .forth - ,    \ r_offset = dlopen-adr 在 .forth 節區內的偏移
6 8 LSHIFT 1 OR ,                 \ r_info = (符號索引 6 << 8) | 類型 1
```

重定位前：
- `dlopen-adr` 變數的記憶體內容為 0（初始值）
- 這意味著加數 A = 0

連結時：
- 連結器發現符號索引 6 對應 `dlopen`（UND，外部未定義）
- 連結器在可執行檔的動態連結資訊中記錄此引用
- 執行時動態載入器將 `dlopen` 的實際位址填入對應位置
...
|--------|------|----------|------|
| 0 | R_386_NONE | 無 | 無操作 |
| 1 | **R_386_32** | **S + A** | **32 位元絕對重定位（SP-Forth 使用）** |
| 2 | R_386_PC32 | S + A - P | 32 位元 PC 相對重定位 |
| 3 | R_386_GOT32 | G + A | 32 位元 GOT 條目 |
| 4 | R_386_PLT32 | L + A - P | 32 位元 PLT 條目 |

> **為何 SP-Forth 使用 R_386_32？**
> 
> SP-Forth 採用「靜態連結 + 延遲動態解析」的混合策略：
> 1. 編譯時：產生 `R_386_32` 重定位項目，符號值設為 0
> 2. 連結時：`gcc` 建立動態連結所需的 PLT/GOT 結構
> 3. 執行時：`dl-init` 在啟動時解析 `dlopen`、`dlsym` 等符號，填入實際位址
>
> 這種設計讓 SP-Forth 可以延遲載入動態程式庫，而不需要在編譯期就知道函數位址。
