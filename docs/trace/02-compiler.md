...
堆疊效果：`( i*x xt -- j*x )`。`EXECUTE` 只消耗 TOS 中的 xt。實作上先把 xt 從 `EAX` 複製到 `EBX`，再把 `[EBP]`（次項）提升為新的 TOS 並調整 `EBP`；這是 TOS-in-EAX 模型下「彈出一項」的正常形式，並不是額外消耗了次項。
...
Forth 字在記憶體中的佈局（相對 NFA 的位移，見 spf_wordlist.f:129-155 的註解）：
  ...
  位移
  -5 ┌─────────────────┐  ← NAME>C：xt cell（4 bytes，NAME> 讀此處）
     │ xt cell (4)      │     通常指向後方的「CALL rel32」執行碼欄位
  -1 ├─────────────────┤  ← NAME>F：flags（1 byte，&IMMEDIATE / &VOC 等）
   0 ├─────────────────┤  ← NFA（nt，名稱欄位位址）
     │ 長度位元組 (1)   │     名稱長度
   1 │ 名稱字元...      │     可變長度
   n ├─────────────────┤  ← NAME>L：LFA（4 bytes，鏈結到前一個字的 nt）
     ├─────────────────┤  ← xt 指向的執行碼欄位
     │ CALL rel32 (5)   │     CFL = 5：CALL 到 runtime code
     │ PFA / 參數資料   │     由 CREATE/VARIABLE/CONSTANT/VALUE 等配置
     └─────────────────┘
```

重點：**flags 在 NFA 前 1 byte（`NAME>F = nt-1`），xt cell 在 NFA 前 5 bytes（`NAME>C = nt-5`），LFA 在名稱字串之後（`NAME>L = nt + len + 1`）**。`NAME>`（`nt-5`）讀出的 xt cell 通常指向後方的 `CALL rel32` 執行碼欄位，而不是緊接在標頭內。
...
3. 第一個被搜尋順序命中的字詞就被採用。搜尋順序優先；而在**同一個詞彙表內**，因為 `+SWORD` 把新字接到 wid 的 head（head-linked list），通常是**較新的定義先被找到**
...
這正是 TOS-in-EAX 模型的精髓：先把舊 TOS 存進堆疊（內聯 `DUP`：`LEA EBP,-4[EBP]; MOV [EBP],EAX`），為 literal 騰出 TOS 位置，再以 `MOV EAX, imm32` 把常值放入 TOS。
...
SHEADER ( addr u -- )
  ...
  └─ 7. HERE SWAP !       回填 xt cell 為 HERE（此時的 HERE 是後續要放 CALL rel32 的執行碼欄位起點）
```

第 5 步的 `+SWORD` 在這裡的行為是：先寫入名稱字串（長度 + 字元），再鏈結到詞彙表的鏈頭。第 7 步回填的是 xt cell，使它指向「接下來要產生的執行碼欄位」；真正的 PFA 會落在該 `CALL rel32` 之後，不是回填當下的 `HERE`。
...
VALUE 的執行體比 CONSTANT 多一條 `CALL _TOVALUE-CODE`。佈局為 `xt+0..+4 = CALL _CONSTANT-CODE`、`xt+5..+8 = value cell`、`xt+9..+13 = CALL _TOVALUE-CODE`。`TO name` 透過 `xt 9 +`（`spf_immed_transl.f:10-26`）精確跳到第三段 `CALL _TOVALUE-CODE`（因為 5+4 = 9），而不是「PFA + CFL」或額外存一個 XT cell。
...
1. **編譯期**：`DOES>`（本身是 IMMEDIATE）依序編譯 `(DOES1)` 和 `(DOES2)` 兩個 XT
2. **使用期（含 DOES> 的 defining word 執行時）**：`CREATE` 先建立新字，接著 defining word 內的 `(DOES1)` 執行，patch 這個**最新建立字**的執行入口，使其日後改走 `(DOES2)`
3. **執行期（被建立的新字日後被呼叫時）**：`(DOES2)` 執行，把 PFA 位址推入資料堆疊，然後跳到 `DOES>` 之後的程式碼
...
**HIDE/SMUDGE 機制**：Forth 的冒號定義在編譯期間是「隱藏」的——它的名稱 counted string 的**第一個字元位元組**（`NAME>CSTRING CHAR+ C@`）被設為 12（特殊標記），使得 `SEARCH-WORDLIST` 找不到它。編譯結束時 `;`（semicolon）呼叫 `SMUDGE` 恢復可見性。

`C-SMUDGE`（USER 變數）暫存原始位元組，用於恢復。`HIDE` 把 `C-SMUDGE` 設為 12 再呼叫 `SMUDGE`；`SMUDGE` 會把名稱 counted string 第一個字元位元組與 `C-SMUDGE` **交換**（不是 XOR）。因此隱藏狀態下該位元組為 12、原字元暫存在 `C-SMUDGE`，再次 `SMUDGE` 時交換回來（`spf_defwords.f:244-257`）。
...
`?SLITERAL1` 的數字解析流程：
1. 檢查前導負號
2. 檢查**尾端**是否為 `.`（`spf_literal.f:10-26` 用 `2DUP CHARS + CHAR- C@ [CHAR] . =`，只看最後一個字元，不是任意位置的小數點）；若 token 以 `.` 結尾，去掉尾端點號並按雙儲存格整數處理
3. 使用 `>NUMBER` 解析
4. 根據是否以 `.` 結尾，決定編譯為 `LITERAL`（單儲存格）或 `2LITERAL`（雙儲存格）
...
其中 `EBX` 是 `C-DO` 保留的原始索引、`EDX` 是它算出的偏移計數器（兩者都不是「暫存器保存值」）。`I` 內聯的 `C-I` 直接 `MOV EAX, [ESP]` 讀 `[ESP+0]` 的目前索引。`LOOP`／`+LOOP` 收尾時用 `LEA ESP, 0C [ESP]` 一次清掉這 3 個 4-byte 項（共 12 = 0x0C 位元組）。
...
: LOOP ( C: do-sys -- )
  ...
  042444FF ,          \ inc dword 4[esp]         — 遞增偏移計數器（供 JNO 判斷）
...
; LOOP 編譯結果：
    inc dword [esp]          ; 索引++
    inc dword 4[esp]         ; 偏移計數器++（用於 +LOOP 的溢位檢測）
...
- `+LOOP` 執行時要求步進值位於 TOS（也就是在迴圈體結尾、執行 `+LOOP` 前推入）；它用該值更新 `[ESP]` 與 `[ESP+4]`，再內聯 `DROP` 丟棄步進值
...
`LEAVE` 的實作是**函式返回**技巧：它以 `lea esp, 08[esp]` 跳過 `[ESP]` 的目前索引與 `[ESP+4]` 的偏移計數器，然後 `RET`——這會跳到 `DO` 時 `PUSH imm32` 推入的 LEAVE 目標位址（恰好是迴圈結束後的位置）。注意跳過的是「索引 + 偏移計數器」，不是傳統的「索引 + 界限」。
...
- `>R` 不產生 `CALL C->R`，而是直接內聯 `C->R` 的資料搬移指令 `PUSH EAX; MOV EAX, [EBP]; LEA EBP, 4[EBP]`（`spf_forthproc.f:1256-1261`）。`JMP EBX` 是 `spf_nonopt.f` 非最佳化版 `>R` 的呼叫返回技巧，不是內聯 `C->R` 的本體
