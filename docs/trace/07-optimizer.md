...
### 1.3 輔助規則檔（macroopt-hide.f）

`macroopt-hide.f`（3919 位元組）是 `macroopt.f` 的輔助規則檔，包含最佳化器的內部實作細節與隱藏字定義：

**主要內容**：
- `OPT-RULES` 迴圈的內部輔助字（如 `OPT-RULE`, `CHECK-RULE`, `APPLY-RULE`）
- `DO_OPT` 的低階實作，負責掃描已編譯的機器碼區段
- 最佳化器狀態變數的內部對應（如 `OPT?` 與內部旗標的映射）
- 條件跳躍最佳化的輔助邏輯（`??BR-OPT` 的實作細節）

**與 macroopt.f 的關係**：
```
macroopt.f（公共介面）
    ├── OPT_INIT, OPT_CLOSE, DO_OPT 等
    └── 呼叫 macroopt-hide.f 的內部字
        ├── OPT-RULES 迴圈控制
        └── 機器碼匹配與替換邏輯
```

這些字通常不直接使用，而是透過 `macroopt.f` 的高階介面（如 `OPT_INIT` / `OPT_CLOSE`）間接呼叫。將輔助規則分離到 `macroopt-hide.f` 的目的是保持 `macroopt.f` 的可讀性，同時提供足夠的低階控制來實作複雜的最佳化邏輯。
...
操作緩衝區（OP Buffer）是一個滑動視窗，記錄最近編譯的機器碼指令的位址。每個插槽佔 2 個 CELL（8 位元組），結構為：
...
每個 OP 插槽的第一個 CELL 指向字典中機器碼的位址（DP），透過 `@` 可以讀取機器碼，透過 `W@`、`C@`、`CELL- @` 等存取不同大小的指令。
...
SetOP 在每次編譯指令後呼叫，將新指令的位址推入 OP 緩衝區。所有舊的操作向後滑動一格（OP1→OP2, OP2→OP3, ...），OP0 記錄最新指令。
...
`?SET` 在最佳化前呼叫，清除所有指向已覆蓋區域的 OP 和 JP 插槽。若 `DP` 已前進超過 `LAST-HERE`（上次最佳化邊界），重設 `:-SET` 和 `J-SET`。
...
: OPT-RULES ( ADDR -- ADDR' FLAG )
  BEGIN
    OP0 @ :-SET U< IF TRUE EXIT THEN    \ 無可最佳化指令，退出
...
| `VALUE V ... V` | `CALL X_doUSER-VALUE ... CALL V` | `MOV EAX, [EDI+offset]` |
...
```forth
: INIT-MACROOPT-LIGHT ( -- )
  084 TO J_COD           \ 0x84 = JNZ rel8 的第二位元組
...
當 `VECT-INLINE?` 為 TRUE 時，`CON>LIT` 也嘗試內聯 VECT 向量呼叫。向量呼叫通常編譯為 `FF 15 [addr]`（CALL [addr]），內聯後直接呼叫目標函數，省去一層間接跳躍。
