...
SP-Forth 使用固定 image address model，例如 POSIX 的 `.forth` 會被安排在 `IMAGE-START` / `IMAGE-BASE` 附近。PIE（position independent executable）會讓載入位址可變，破壞這種固定位址與 relocation 假設。因此建構流程會偵測 GCC 是否預設 PIE，必要時加上 `-no-pie`。
...
1. [06-build-save.md §1](06-build-save.md#1-建構系統makefile-深入解析)：先理解 `spf4` / `spf4e` 怎麼被建出來。
