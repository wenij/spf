...
在 Windows 系統上，映像基址是固定的（`IMAGE-START = 0x8050000`），因此位址轉換是多餘的（`>VIRT` 是恆等操作）。
...
這行在所有目標代碼編譯完成後執行，將 `virt-offset` 設定為 `IMAGE-START - HERE`（更一般地說是 `va - HERE`）。此後所有 `>VIRT` 轉換都基於這個偏移量。
...
這裡的 `addr` 應理解為**要寫進 target 映像的目標位址**。對同一映像內的呼叫來說，`TC-CALL,` 先按當前 target 位置計算 `rel32`；若後續儲存為 POSIX `spf4.o`，最終位址仍可再由 ELF 重定位流程調整。
...
初始偏移 16 bytes 为核心系統 USER 變數（如 `TIB`、`#TIB`、`>IN` 等）預留空間。
...
1. **最佳化路徑**（`OP0 @ C@ 0xB8 =`）：如果前一條指令是 `MOV EAX, #imm32`（常值），表示 THROW 的引數是一個已知常數。在這種情況下，可以直接用 `JMP` 跳到 THROW 處理程式碼，省去條件判斷。

   ```asm
   ; 最佳化路徑（TOS 為常數，非零即跳到 THROW）：
   MOV EAX, #error_code    ; 已由 TC-LIT, 產生
   JMP THROW_HANDLER       ; 無條件跳轉
   ```

2. **一般路徑**：產生 `OR EAX, EAX`（測試 TOS）+ `JNZ near THROW_HANDLER`（條件跳轉）+ `DROP`（丟棄 TOS 若為零）。
...
TRUE CONSTANT CON>LIT            \ 常數摺疊永遠啟用
...
- `OPT?` = FALSE：最佳化器永不啟用
- `INLINE?` = 0：所有字都不可內聯
- `CON>LIT` = TRUE：常數摺疊仍然啟用（因為這是正確性需求）
...
noopt.f 只保留 `CON>LIT` 和基本的 `INLINE,` 機制，確保編譯器在無最佳化模式下仍能正常工作。
