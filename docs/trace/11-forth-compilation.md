...
> **注意（與標準 Forth 不同）**：SP-Forth 的 `:` **不是** IMMEDIATE — 定義以單純的 `;` 結尾，沒有 `; IMMEDIATE`。SP-Forth 仰賴 `:` 在直譯狀態下被呼叫；標準 Forth 慣例上的 IMMEDIATE 對 `:` 在 SP-Forth 並不適用。
...
**Q: 為什麼 SP-Forth 的 `:` 不需要是 IMMEDIATE？**

A: 標準 Forth 把 `:` 設為 IMMEDIATE 是為了「在編譯狀態中遇到 `:` 時仍能執行它去開新定義」。SP-Forth 採用較嚴格的模型：`:` 預期在直譯狀態下被呼叫，巢狀定義（在另一個 `:` 體內再寫 `:`）並不是受支援的常見用法。若要在編譯期建立 noname 定義，請改用 `:NONAME`（`spf_defwords.f:261–278`）。
