...
- `ANSI><OEM` 在 POSIX 上為空操作（NOOP），因為 UNIX 系統統一使用同一字元編碼
- `KEY` 和 `KEY?` 初始為 FALSE，表示預設不支援互動式鍵盤輸入，需由終端機驅動（如 `lib/include/ansiterm.f`）替換
...
`H-STDOUT DUP 0 >` 檢查：若 H-STDOUT ≤ 0（例如在無終端機的daemon模式下），直接丟棄輸出而不報錯。
...
注意到 `UNIX-LINES` 將 `0A0A` 寫入 LT（小端序存儲為 `0A 0A`），但實際使用時只取前 `LTL @` = 1 位元組，即 `0A`。這是因為 CELL 是 4 位元組但 `EOLN` 只回傳前 `LTL @` 個位元組。
...
- Windows：使用 `SetUnhandledExceptionFilter` 或 Vectored Exception Handling
...
所有檔案操作都使用 POSIX 64 位元呼叫（`open64`、`lseek64`、`fstat64`、`ftruncate64`），支援大檔案。
...
若找不到 SPF.ERR 檔案，回傳格式化字串 `"[-]nnnn ERROR #"`。
...
全域變數，用於防止 DUMP-TRACE 的遞迴呼叫。若在 DUMP-TRACE 執行期間又觸發信號，第二次的 DUMP-TRACE 會直接退出。
...
- `<SET-EXC-HANDLER>`：Windows 使用結構化例外處理（SEH/Vectored EH），POSIX 使用 sigaction
...
8 CELLS + 80 + 11 CELLS = 124 位元組：這是跳過 `EXCEPTION_RECORD` 和 `CONTEXT` 結構的浮點暫存器區塊，到達通用暫存器區域的偏移。數字 80 是 `FLOATING_SAVE_AREA` 的大小。
...
```forth
VARIABLE MAINX          \ 主程式執行標記（0=互動模式，非0=執行後退出）
...
此字接收從 `ucontext_t` 或 EXCEPTION_RECORD 中提取的三個暫存器值，用於重建堆疊追蹤：
...
Windows 版使用 `GetStdHandle` API 取得標準 I/O 控制代碼，並在 GUI 模式下檢查 stdout 是否為無效控制代碼（0x10001 = `INVALID_HANDLE_VALUE` 的近似值）。
...
硬體例外（SIGSEGV/SIGFPE 等）
  ...
  │
  ├─ Windows: SEH/Vectored EH → EXC-DUMP1 → DUMP-TRACE-USING-REGS
  │
Forth 例外（THROW n）
  │
  ├─ n=0: 丟棄，繼續
