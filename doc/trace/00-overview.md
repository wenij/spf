...
| 先備知識 | 為何需要 |
|----------|----------|
| ANS Forth 94 基本語意 | 理解 `IMMEDIATE`、`STATE`、詞彙表、`CREATE ... DOES>`、`THROW/CATCH` |
| IA-32（x86 32 位元）組合語言 | 理解 `CALL rel32`、`RET`、`EAX/EBP/ESP/EDI` 的角色 |
...
2. `01-kernel.md`：理解 TOS-in-EAX、定義字與核心原語。
3. `02-compiler.md`：理解 parser / search-order / interpreter / compiler。
4. `03-cross-compiler.md`：理解 host / target / meta-compilation。
5. `07-optimizer.md`：理解最佳化器如何介入目標機器碼生成。
6. `04-posix-platform.md`：理解 POSIX 平台抽象、FFI、執行緒與信號。
7. `05-io-error-init.md`：理解 I/O、例外、初始化與互動式迴圈。
8. `06-build-save.md`：最後再看建構與映像輸出細節，最容易對上前面概念。
