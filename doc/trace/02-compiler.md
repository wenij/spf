...
**STATE 匹配規則**：
- `STATE @ =` 為 true 時，表示 **字詞的立即性**（旗標 -1 或 1）與 **編譯狀態**（STATE 為 -1）匹配
- 具體來說：
  - 編譯模式下（STATE = -1），立即字（旗標 1）的 `STATE @ = 1` 為真 → `COMPILE,`
  - 編譯模式下，非立即字（旗標 -1）的 `STATE @ = -1` 也為真 → 但等等，這不對...

事實上，`STATE @` 的值在 Forth 94 標準中只定義為「真」和「假」，`SFIND` 回傳 1 表示立即字、-1 表示非立即字。`STATE @ =` 的巧妙之處在於：
- 編譯模式（STATE = true/-1）：立即字（旗標 1）→ `true = 1` → false → `EXECUTE`；非立即字（旗標 -1）→ `true = -1` → true → `COMPILE,`
- 直譯模式（STATE = 0/false）：立即字（旗標 1）→ `false = 1` → false → `EXECUTE`；非立即字（旗標 -1）→ `false = -1` → true → `EXECUTE`

等一下，讓我重新理解。在 SP-Forth 中 `STATE @` 在編譯模式下為 `-1`（TRUE），直譯模式下為 `0`（FALSE）。SFIND 回傳 `1` 表示立即字，`-1` 表示非立即字。

所以 `STATE @ =` 的比較：
- 編譯模式 + 非立即字：`-1 = -1` → true → `COMPILE,`（正確：編譯非立即字）
- 編譯模式 + 立即字：`-1 = 1` → false → `EXECUTE`（正確：立即執行立即字）
- 直譯模式 + 任何：`0 = 1` 或 `0 = -1` → 都為 false → `EXECUTE`（正確：直譯模式總是執行）
