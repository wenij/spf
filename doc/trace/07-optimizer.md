...
這表示 `DUP` 先把原本的 TOS 寫回資料堆疊，再由 `+` 立刻從 `[EBP]` 讀回同一值。最佳化器能利用這種「剛寫入又立刻讀回」的短命堆疊流量做 peephole rewrite，但實際能否縮成更短序列，仍取決於前後文與當前追蹤到的 EAX / EBP 狀態。比較安全的理解方式是：它在消去冗餘的 stack traffic，而不是固定改寫成某一條單獨的 `MOV` / `ADD` 模式。
